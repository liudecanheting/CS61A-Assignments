...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    first_value=strategy(0,0)
    for x in range(0,goal):
        for y in range(0,goal):
            if strategy(x,y)!=first_value:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max_score=0
    while i<=10:
        now_score=make_averaged(roll_dice,times_called)(i,dice)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    if score-original_score>=threshold:
        return 0
    return num_rolls
